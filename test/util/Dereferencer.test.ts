...
describe('Test ObjectRefTraverser', () => {
  ...
  test('Test Local', async () => {
    ...
    expect(traverser.get(obj).obj).toEqual(obj);
    expect(traverser.get(obj.b).obj).toEqual("x");
  });
  ...
  test('Expect Exception From Unresolved', async () => {
    ...
    expect(() => traverser.get(obj.b)).toThrow();
  });
  ...
  test('Test External', async () => {
    ...
    expect(traverser.get(obj.a_c).obj).toEqual("y");
  });
  ...
  test('URL, w/ missing dir/c.json', async () => {
    ...
    try {
      await Dereferencer.create<any>('https://omnigen.fake.localhost/a');
  ...
  test('URL, w/ existing mocked files', async () => {
    ...
    const traverser = await Dereferencer.create<any>('https://omnigen.fake.localhost.com/a');

    const c = traverser.get(traverser.getFirstRoot().a_e);
    expect(c.obj.c_a.value).toEqual("z");
