...
import {z} from 'zod';
import {DocVisitorUnknownTransformer, safeSet, visitUniformArray, visitUniformObject} from './helpers';
import {JSONSchema9} from '../definitions';

const logger = LoggerFactory.create(import.meta.url);

export function createDefaultJsonSchema9Visitor(): JsonSchema9Visitor {
  ...
  return {
    ...
    schema_option: (option, visitor) => {
      const res = visitor.schema_definition(option.value, visitor);
      return res ? {idx: option.idx, value: res} : undefined;
    },
    jsonSchemaTypeName: v => v,
    additionalItems: (v, visitor) => v ? visitor.schema_definition(v, visitor) : v,
    additionalProperties: (v, visitor) => v ? visitor.schema_definition(v, visitor) : v,
    ...
    not: (v, visitor) => v ? visitor.schema_definition(v, visitor) : v,
    ...
    dependentSchemas: (v, visitor) => v ? visitUniformObject(v, (it, k) => visitor.dependent_schema({key: k, value: it}, visitor)) : v,
    dependent_schema: (v, visitor) => {
      const res = visitor.schema_definition(v.value, visitor);
      return res ? {key: v.key, value: res} : undefined;
    },
    ...
    items_item: (v, visitor) => typeof v == 'boolean' ? v : visitor.schema(v, visitor),
    minItems: v => v,
    maxItems: v => v,
    minLength: v => v,
    maxLength: v => v,
    ...
    multipleOf: v => v,
    pattern: v => v,
    patternProperties: (v, visitor) => v ? visitUniformObject(v, it => visitor.schema_definition(it, visitor)) : v,
    ...
    contains: (v, visitor) => v ? visitor.schema_definition(v, visitor) : v,
    examples: (v, visitor) => v ? visitUniformArray(v, item => visitor.jsonSchemaType(item, visitor)) : v, // visitor.jsonSchemaType(v, visitor) : v,
    propertyNames: (v, visitor) => v ? visitor.schema_definition(v, visitor) : v,
    ...
    readOnly: v => v,
    writeOnly: v => v,
    if: (v, visitor) => v ? visitor.schema_definition(v, visitor) : v,
    then: (v, visitor) => v ? visitor.schema_definition(v, visitor) : v,
    else: (v, visitor) => v ? visitor.schema_definition(v, visitor) : v,
  };
}

export const DefaultJsonSchema9Visitor: JsonSchema9Visitor = createDefaultJsonSchema9Visitor()
