...
describe('object conversions', () => {
  ...
  test('basic', () => {
    ...
    expect(result.compressSoloReferencedTypes).toEqual(true);
    expect(result.compressUnreferencedSubTypes).toEqual(true);
    expect(result.allowCompressInterfaceToInner).toEqual(false);
    expect(result.additionalPropertiesInterfaceAfterDuplicateCount).toEqual(3);
    expect(result.compressTypesLevel).toEqual(CompressTypeLevel.FUNCTIONALLY_SAME);
    expect(result.compressTypeNaming).toEqual(CompressTypeNaming.EXACT);
  });
...
describe('plugin path', () => {
  ...
  test('match 2 steps, replace', async () => {
  ...
  test('match 2 steps, 1 runtime', async () => {
  ...
  test('match and rank by score, without runtime/pathing', async () => {
    ...
    expect(result.results[2].ctx).toMatchObject({d: '0123'});
    expect(result.results[2].ctx).not.toHaveProperty('e');
    expect(result.results[3].ctx).toMatchObject({d: '0123', e: '014'});
  });

  test('match and rank by score, with runtime/pathing', async () => {
  ...
  test('match and rank by score, with multiple matching runtime/pathing', async () => {
